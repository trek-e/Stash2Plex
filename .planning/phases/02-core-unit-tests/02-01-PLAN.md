...
must_haves:
  truths:
    ...
    - "create_sync_job generates valid SyncJob with all required fields"
  artifacts:
  ...
  key_links:
    ...
    - from: "tests/sync_queue/test_operations.py"
      to: "sync_queue/models.py"
      via: "imports create_sync_job for direct testing"
      pattern: "from sync_queue.models import"
---

<objective>
Create comprehensive unit tests for the sync_queue module (QueueManager, operations, models, DeadLetterQueue).

Purpose: Test queue lifecycle, job operations, job creation, and failed job storage with SQLite isolation.
...
Also add tests for create_sync_job from models.py (for explicit coverage):
- test_create_sync_job_returns_syncjob_dict: Verify return type is dict with correct keys
- test_create_sync_job_sets_scene_id: Verify scene_id field matches input
- test_create_sync_job_sets_update_type: Verify update_type field matches input
- test_create_sync_job_sets_data: Verify data field matches input
- test_create_sync_job_sets_enqueued_at: Verify enqueued_at is a float timestamp (use time.time() comparison)
- test_create_sync_job_sets_job_key: Verify job_key is f"scene_{scene_id}"
...
Use mocker to mock queue methods where needed.
  ...
  <done>All QueueManager, operations, and create_sync_job tests pass, covering initialization, job operations, stats, and timestamps</done>
</task>
...
- models.py: Should be fully covered by create_sync_job tests
...
5. models.py (create_sync_job) explicitly tested
</verification>

<success_criteria>
- tests/sync_queue/test_manager.py exists with TestQueueManager class
- tests/sync_queue/test_operations.py exists with operation tests AND create_sync_job tests
